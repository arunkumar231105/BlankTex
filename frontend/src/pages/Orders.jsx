...
export default function Orders() {
  ...
  useEffect(() => {
    ...
    toast.success(`Order ${orderNo} sent to supplier and saved`);
    ...
    <>
      ...
      <div className="card orders-card">
        ...
        {loading ? <div className="loading">Loading orders…</div> : !orders.length ? <div className="empty"><div className="big">📦</div>No orders found.</div> : (
          <div className="tbl-wrap"><table className="tbl orders-table"><thead><tr><th>Order ID</th><th>Customer</th><th>Location</th><th>Items</th><th>Status</th><th>Updated</th><th /></tr></thead><tbody>
            {orders.map((order) => <tr key={order.order_no} className="row-click" onClick={() => openOrder(order.order_no)}>
              <td><b className="order-id">{order.order_no}</b></td><td>{order.recipient_name || '—'}</td><td>{[order.city, order.state_province].filter(Boolean).join(', ') || '—'}</td><td>{order.item_count || 0}</td><td><Status value={order.supplier_status} text={order.supplier_status_str} /></td><td className="orders-muted">{dateTime(order.updated_at)}</td><td>›</td>
            </tr>)}
          </tbody></table></div>
        )}
      </div>
      ...
      {selected && <><div className="order-drawer-backdrop" onClick={() => setSelected(null)} /><aside className="order-drawer">
        {detailLoading ? <div className="loading">Loading order…</div> : <>
          <div className="order-drawer-head"><div><small>ORDER ID</small><h2>{selected.order_no}</h2><Status value={selected.supplier_status} text={selected.supplier_status_str} /></div><button onClick={() => setSelected(null)}>×</button></div>
          ...
          <div className="order-drawer-foot"><div><button className="btn" onClick={() => sync([selected.order_no])} disabled={syncing}>↻ Sync Status</button><button className="btn" onClick={getTracking}>📦 Tracking</button></div><div><button className="btn" onClick={() => setShippingOpen(true)}>✎ Update Shipping</button>{![13, 15].includes(Number(selected.supplier_status)) && <button className="btn danger" onClick={closeOrder} disabled={saving}>× Close Order</button>}</div></div>
        </>}
      </aside></>}
